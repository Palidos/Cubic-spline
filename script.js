...
inpPlot.addEventListener('click', () => {
	...
	let steps = [];
	inpSteps.value ? steps.push(+inpSteps.value) : steps.push(2);
	...
	for (let n of steps) {
		...
		//   let simpleFunction = `${spline.a} + ${spline.b} * (x - ${spline.x}) + ${
		//     spline.c} / 2 * (x - ${spline.x}) * (x - ${spline.x}) + ${spline.d} / 6 * (x - ${spline.x}) * (x - ${spline.x}) * (x - ${spline.x})`;
		//   funcOptions.data.push({
		//     fn: simpleFunction,
		//     color: 'red',
		//     range: [splines[i-1].x, spline.x],
		//   })
		// }
		...
		for (let i = 1; i < splines.length; i++) {
			let dx, curVal;
			// console.log(`a: ${splines[i].a}, b: ${splines[i].b}, c: ${splines[i].c}, d: ${splines[i].d}`);
			for (let x = splines[i - 1].x; x <= splines[i].x; x += epsilon) {
				...
				precision = Math.max(Math.abs(curVal - math.evaluate(func, { x: x })), precision);
			}
		}
buildTable(splines);
		funcOptions.data.push({
	...
	console.log(`Interpolation error: ${precision}`);
